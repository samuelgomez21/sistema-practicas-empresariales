...
const schema = z.object({
  ...
  contactoEmergencia: z.string().min(5, 'Ingresa nombre y teléfono'),
  programa:           z.string().min(1, 'Selecciona un programa'),
  semestre:           z.coerce.number().min(1).max(10),
  numeroPractica:     z.coerce.number().min(1).max(5),
  creditosAprobados:  z.coerce.number().min(0),
  promedioAcumulado:  z.coerce.number().min(0).max(5),
})

export default function ModalEstudiante({ estudiante, programasDisponibles, onClose, onGuardado }) {
  const esEdicion = !!estudiante

  const { register, handleSubmit, formState: { errors } } = useForm({
    resolver: zodResolver(schema),
    defaultValues: estudiante ?? { semestre: 8, numeroPractica: 1 },
  })

  const mutation = useMutation({
    mutationFn: (data) =>
      esEdicion
        ? usuariosApi.editarEstudiante(estudiante.id, data)
        : usuariosApi.crearEstudiante(data),
    onSuccess: () => {
      toast.success(esEdicion ? 'Estudiante actualizado' : 'Estudiante creado — se envió contraseña temporal al correo')
      onGuardado()
    },
  ...
  const Campo = ({ id, label, error, children }) => (
          ...
          <div>
            ...
            {!esEdicion && (
              <p className="text-[10px] mt-0.5" style={{ color: '#8a9bb0' }}>
                Se generará contraseña temporal y se enviará al correo registrado
              </p>
            )}
          </div>
        ...
        <form onSubmit={handleSubmit(d => mutation.mutate(d))} className="flex flex-col gap-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Campo label="Tipo de documento" error={errors.tipoDocumento}>
                <select {...register('tipoDocumento')} className={ic} style={is}>
                  <option value="">Seleccionar...</option>
                  {TIPOS_DOC.map(t => <option key={t} value={t}>{t}</option>)}
                </select>
              </Campo>
              <Campo label="Número de documento" error={errors.documento}>
                <input {...register('documento')} className={ic} style={is} placeholder="Número de documento" />
              </Campo>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Campo label="Programa" error={errors.programa}>
                <select {...register('programa')} className={ic} style={is}>
                  <option value="">Seleccionar programa...</option>
                  {programasDisponibles.map(p => <option key={p} value={p}>{p}</option>)}
                </select>
              </Campo>
              <Campo label="Semestre actual" error={errors.semestre}>
                <input {...register('semestre')} type="number" min="1" max="10"
                  className={ic} style={is} />
              </Campo>
              <Campo label="Número de práctica" error={errors.numeroPractica}>
                <input {...register('numeroPractica')} type="number" min="1" max="5"
                  className={ic} style={is} />
              </Campo>
