...
export default function TutoresAdminPage({ empresaIdFijo } = {}) {
  ...
  const [filtroEmp,  setFiltroEmp]  = useState(empresaIdFijo ?? '')
  ...
  const { data: tutores = [], isLoading } = useQuery({
    queryKey: ['tutores', filtroEmp],
    queryFn:  () => empresasApi.getTutoresByEmpresa(filtroEmp || null),
    enabled:  true,
  })

  const toggleMutation = useMutation({
    mutationFn: (id) => empresasApi.toggleTutor(id),
    onSuccess: () => {
        ...
        <table className="w-full">
          <thead>
            <tr style={{ background: '#f7f9fb' }}>
              {['Tutor', 'Cargo', 'Empresa', 'Teléfono', 'Estado', 'Acciones'].map(h => (
      ...
      {(modal || editando) && (
        <ModalTutor
          ...
          onGuardado={() => {
            ...
            setModal(false)
            setEditando(null)
            toast.success(editando ? 'Tutor actualizado' : 'Tutor creado')
          }}
        />
      )}
    </div>
