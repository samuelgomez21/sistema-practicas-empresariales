...
export default function MisEstudiantesPage() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-3">
        {estudiantes.map(e => {
          const cfg = ESTADO_PRACTICA_LABEL[e.practica?.estado] ?? ESTADO_PRACTICA_LABEL.ASIGNADA_PENDIENTE_INICIO
          ...
          return (
            <button key={e.id}
              ...
              <div className="flex items-center gap-2">
                {e.practica?.avancesPendientes > 0 && (
                  <span className="text-[9px] font-bold px-2 py-1 rounded-full"
                    style={{ background: '#fff8e6', color: '#a07010' }}>
                    {e.practica.avancesPendientes} avance(s) por revisar
                  </span>
                )}
                <span className="text-[9px] font-bold px-2 py-1 rounded-full"
