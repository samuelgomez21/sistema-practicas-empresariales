...
export default function ModalDocente({ docente, onClose, onGuardado }) {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm({
    ...
    defaultValues: docente ? { nombre: docente.nombre, email: docente.correo } : {},
  })

  const mutation = useMutation({
    mutationFn: (data) =>
      esEdicion
        ? usuariosApi.actualizarDocente(docente.id, { ...data, activo: docente.activo })
        : usuariosApi.crearDocente(data),
    onSuccess: () => {
      toast.success(esEdicion ? 'Docente actualizado' : 'Docente creado correctamente')
      onGuardado()
    },
    onError: (err) => {
      const msg = err?.response?.data?.message ?? 'Error al guardar el docente'
      toast.error(msg)
    },
  })
      ...
      <div className="bg-white rounded-xl w-full max-w-md p-6"
        ...
        <div className="flex items-center justify-between mb-5">
          <p className="text-sm font-bold" style={{ color: '#023859' }}>
            {esEdicion ? 'Editar docente' : 'Nuevo docente asesor'}
          </p>
        ...
        <form onSubmit={handleSubmit(d => mutation.mutate(d))} className="flex flex-col gap-3">
          ...
          <div>
            ...
            <input {...register('email')} type="email" className={ic} style={is}
              placeholder="docente@universidad.edu.co" disabled={esEdicion} />
          ...
          {!esEdicion && (
            <div className="p-2.5 rounded-lg text-[10px]"
              ...
              El docente recibirá una contraseña temporal por correo y deberá cambiarla en su primer inicio de sesión.
            </div>
          )}
          ...
          <div className="flex gap-2 justify-end pt-2">
            ...
            <button type="submit" disabled={mutation.isPending}
              ...
              {mutation.isPending ? 'Guardando...' : esEdicion ? 'Guardar cambios' : 'Crear docente'}
            </button>
          </div>
        </form>
      </div>
