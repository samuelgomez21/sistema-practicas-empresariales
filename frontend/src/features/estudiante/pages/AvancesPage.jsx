...
export default function AvancesPage() {
  ...
  const mutation = useMutation({
    mutationFn: async (data) => {
      setSubiendo(true)
      try {
        return await estudianteApi.subirAvanceConArchivo({
          ...data,
          archivo: archivo,
        })
      } finally {
    ...
    onError: (err) => toast.error(err?.response?.data?.message ?? 'Error al registrar el avance'),
  })

  // Agrupar avances por corte
  const cortes = practica?.cortes ?? []
  const avancesPorCorte = cortes.length > 0
    ? cortes.map(c => ({ corte: c, avances: avances.filter(a => a.corteNumero === c.numero) }))
    : [{
        corte: { numero: 1, nombre: 'Avances', fechaLimite: null },
        avances,
      }]

  const totalAvances = avances.length
  const revisados    = avances.filter(a => ['REVISADO', 'APROBADO'].includes(a.estado)).length
            ...
            <div key={corte.numero}>
              ...
              {av.length === 0 ? (
                ...
                <div className="ml-9 flex flex-col gap-2">
                  {av.map(a => {
                    const cfg = ESTADO_AVANCE_STYLE[a.estado] ?? ESTADO_AVANCE_STYLE.PENDIENTE
                    return (
                        ...
                        <div className="flex items-start justify-between mb-2">
                          <div>
                            ...
                            <p className="text-[10px]" style={{ color: '#8a9bb0' }}>
                              {a.fechaEntrega
                                ? `Entregado: ${new Date(a.fechaEntrega).toLocaleDateString('es-CO')}`
                                : 'Sin entregar'}
                            </p>
                          </div>
                          ...
                          {a.comentarioDocente && (
                            <div className="w-full mt-2 p-2 rounded-lg flex gap-2"
                              ...
                              <div>
                                ...
                                <p className="text-[10px]" style={{ color: '#1a5c38' }}>{a.comentarioDocente}</p>
                              </div>
                            </div>
                          )}
                        </div>
            ...
            <form onSubmit={handleSubmit(d => mutation.mutate(d))} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <select {...register('corteNumero')} className={ic} style={is}>
                  {(practica?.cortes ?? [{ numero: 1, nombre: 'Corte 1' }]).map(c => (
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <button type="button" onClick={() => fileRef.current?.click()}
                  ...
                  style={{
                    background: '#f7f9fb',
                    border: '1.5px dashed #dce4ec',
                    color: archivo ? '#1a7a4a' : '#8a9bb0',
                  }}>
                  <Upload size={14} />
                  {archivo ? archivo.name : 'Seleccionar archivo PDF o Word'}
                </button>
              </div>
              ...
              <div className="flex gap-2 justify-end pt-1">
                ...
                <button type="submit" disabled={mutation.isPending || subiendo}
                  ...
                  style={{ background: mutation.isPending || subiendo ? '#a0aab4' : '#D91438' }}>
                  {subiendo ? 'Subiendo archivo...' : mutation.isPending ? 'Guardando...' : 'Registrar avance'}
                </button>
              </div>
            </form>
          </div>
