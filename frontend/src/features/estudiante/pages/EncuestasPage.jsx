...
export default function PazYSalvoPage() {
  ...
  if (isLoading) return (
    <div className="bg-white rounded-xl p-5 animate-pulse" style={{ border: '0.5px solid #e2e8f0' }}>
      {[1,2,3,4,5].map(i => <div key={i} className="h-8 bg-gray-50 rounded mb-2" />)}
    </div>
  )
  ...
  const tienePaz     = completados === total
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-white rounded-xl p-6 flex items-center gap-5"
        ...
        <div className="flex-1">
          <h2 className="text-base font-bold" style={{ color: tienePaz ? '#1a7a4a' : '#c0392b' }}>
          ...
          <div className="mt-3 flex items-center gap-3">
            <div className="flex-1 h-2 rounded-full" style={{ background: '#f0f2f5' }}>
              <div className="h-2 rounded-full transition-all"
                style={{ width: `${pct}%`, background: tienePaz ? '#1a7a4a' : '#0B416B' }} />
            </div>
            <span className="text-xs font-bold" style={{ color: tienePaz ? '#1a7a4a' : '#023859' }}>
              {completados}/{total}
            </span>
          </div>
        </div>
